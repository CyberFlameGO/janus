...
use janus_core::{
    message::Duration,
    test_util::kubernetes::{Cluster, PortForwardGuard},
    time::RealClock,
    TokioRuntime,
};
use janus_server::{
...
pub enum Janus {
    ...
    KubernetesCluster {
        port_forwards: Vec<PortForwardGuard>,
    },
}
...
impl Janus {
    ...
    where
        P: AsRef<Path>,
    {
        let cluster = Cluster::create(kubeconfig_path, kubernetes_context_name);
        ...
        let _datastore_port_forward =
            cluster.forward_port(namespace, "postgresql", local_db_port, 5432);
        ...
        let aggregator_port_forward =
            cluster.forward_port(namespace, "aggregator", aggregator_local_port, 80);
